...
void main() {
    ...
    int WINDOW_SIZE = min(MAX_WINDOW_SIZE, int(resolution.y) / shrink_factor + (int(resolution.y) / shrink_factor) % 2);
    ...
    for(int i = -KERNEL_SIZE; i <= KERNEL_SIZE; i++) {
        acc += kernel[KERNEL_SIZE + i] * texture(ssr_sampler, tex_coord + (0.0, vec2(float(i)) / resolution)).rgb;
    }
